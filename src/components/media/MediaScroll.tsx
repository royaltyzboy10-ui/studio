...
const MOCK_HIGHLIGHTS = [
  { 
    id: 1, 
    title: "Unreal Dunk from Anthony Edwards", 
    user: "@NBA", 
    likes: "125k", 
    comments: "2k", 
    imageId: "dunk-highlight",
    hint: "basketball dunk"
  },
  { 
    id: 2, 
    title: "Champions League Masterclass", 
    user: "@ChampionsLeague", 
    likes: "89k", 
    comments: "1.2k", 
    imageId: "soccer-goal-highlight",
    hint: "soccer goal"
  },
  { 
    id: 3, 
    title: "F1 High-Speed Overtake", 
    user: "@F1", 
    likes: "210k", 
    comments: "5.5k", 
    imageId: "f1-highlight",
    hint: "formula one"
  },
];
  ...
  return (
    <div className="w-full h-[700px] overflow-y-scroll snap-y snap-mandatory scrollbar-hide bg-secondary/20 rounded-2xl border border-white/5">
      {MOCK_HIGHLIGHTS.map((video) => {
        const placeholder = PlaceHolderImages.find(img => img.id === video.imageId);
        return (
          <div key={video.id} className="relative w-full h-full snap-start flex items-center justify-center bg-black overflow-hidden group">
            <Image 
              src={placeholder?.imageUrl || `https://picsum.photos/seed/${video.id}/400/700`} 
              alt={video.title} 
              fill 
            ...
            <div className="absolute bottom-0 left-0 p-6 w-full flex items-end justify-between">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  <Badge className="bg-primary text-primary-foreground font-headline font-bold">{video.user}</Badge>
                  <span className="text-white font-headline font-bold">Trending Highlights</span>
                </div>
